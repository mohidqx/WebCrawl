...
Parameters: {len(parsed.query.split('&')) if parsed.query else 0}
    ...
    def log(self, message, tag="INFO"):
        """Queue log message to main thread"""
        self.after(0, lambda: self._log_main_thread(message, tag))
    
    def _log_main_thread(self, message, tag="INFO"):
        """Update console on main thread"""
        try:
    ...
    def export_results(self):
        ...
        export_thread = threading.Thread(target=self._export_thread, daemon=True)
        export_thread.start()
...
💡 Recommendation: Review sensitive files immediately"""
    ...
    def smooth_progress_update(self, target_value, steps=20, delay=10):
        ...
        self.after(0, lambda: self._smooth_progress_update_main(target_value, steps, delay))
    
    def _smooth_progress_update_main(self, target_value, steps=20, delay=10):
        """Main thread progress update animation"""
        try:
    ...
    def smooth_banner_status(self, text, fade_steps=3):
        ...
        self.after(0, lambda: self._smooth_banner_status_main(text, fade_steps))
    
    def _smooth_banner_status_main(self, text, fade_steps=3):
        """Main thread banner status update"""
        try:
    ...
    def run_crawl_with_options(self, url, depth, threads):
        ...
        try:
            ...
            # Run crawl in background thread to prevent GUI freezing
            crawl_thread = threading.Thread(target=self._run_crawl_thread, args=(url, depth, threads), daemon=True)
            crawl_thread.start()
    ...
    def _run_crawl_thread(self, url, depth, threads):
        """Wrapper thread for crawl operations with proper error handling"""
        try:
    ...
    def populate_results_tree(self, results):
        """Populate categorized result lists - queue on main thread"""
        self.after(0, lambda: self._populate_results_tree_main_thread(results))
    
    def _populate_results_tree_main_thread(self, results):
        """Populate categorized result lists on main thread"""
        # Clear all listboxes
        self.sensitive_listbox.delete(0, tk.END)
        self.dir_listbox.delete(0, tk.END)
        self.file_listbox.delete(0, tk.END)
    ...
    def update_statistics(self, results):
        # Queue statistics update on main thread
        self.after(0, lambda: self._update_statistics_main_thread(results))
    
    def _update_statistics_main_thread(self, results):
...
"""
    ...
    def create_charts(self, results):
        # Queue chart creation on main thread to avoid threading issues
        self.after(0, lambda: self._create_charts_main_thread(results))
    
    def _create_charts_main_thread(self, results):
        """Create charts on main thread"""
...
{str(stats.get('status_codes', {})).replace('{', '+- ').replace('}', '')}
    ...
    def count_urls(self):
        ...
        analysis_thread = threading.Thread(target=self._count_urls_thread, daemon=True)
        analysis_thread.start()
    
    def _count_urls_thread(self):
        """Background thread for counting URLs"""
        try:
            if hasattr(self, 'current_results') and self.current_results:
    ...
    def find_duplicates(self):
        ...
        analysis_thread = threading.Thread(target=self._find_duplicates_thread, daemon=True)
        analysis_thread.start()
    ...
    def analyze_file_types(self):
        ...
        analysis_thread = threading.Thread(target=self._analyze_file_types_thread, daemon=True)
        analysis_thread.start()
    
    def _analyze_file_types_thread(self):
        """Background thread for file type analysis"""
        try:
            if hasattr(self, 'current_results') and self.current_results:
                urls = self.current_results.get('urls', [])
                file_types = {}
                for url in urls:
                    ext = url.split('?')[0].split('#')[0].split('.')[-1].lower() if '.' in url.split('?')[0] else 'no-ext'
    ...
    def analyze_domains(self):
        ...
        analysis_thread = threading.Thread(target=self._analyze_domains_thread, daemon=True)
        analysis_thread.start()
    
    def _analyze_domains_thread(self):
        """Background thread for domain analysis"""
        try:
            from urllib.parse import urlparse
            if hasattr(self, 'current_results') and self.current_results:
                urls = self.current_results.get('urls', [])
    ...
    def analyze_performance(self):
        ...
        analysis_thread = threading.Thread(target=self._analyze_performance_thread, daemon=True)
        analysis_thread.start()
    
    def _analyze_performance_thread(self):
        """Background thread for performance analysis"""
        try:
            if hasattr(self, 'current_results') and self.current_results:
    ...
    def analyze_threats(self):
        ...
        analysis_thread = threading.Thread(target=self._analyze_threats_thread, daemon=True)
        analysis_thread.start()
    
    def _analyze_threats_thread(self):
